...
\usepackage{amssymb, amsfonts,mathtools,bbm}
...
%\def\betA{\begin{table}}\def\eetA{\end{table}}
...
\def\ASD{acyclic siphon decomposition}
...
\def\COT{chemical organization theorem}
...
\def\GLV{Generalized Lotka-Volterra model}\def\gLV{generalized Lotka-Volterra model}
\def\IG{\text{Invasion Graph}}
\def\IGMS{interaction graph on minimal siphons}
...
\def\Mm{Max–Min}
\def\num{numerator}
\def\rap{rational approximation} 