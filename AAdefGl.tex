...
\usepackage{listings,verbatim,cases,hyperref}
...
\newcommand{\beD}[1][]{
  ...
  \else
    ...
    \newglossaryentry{d\thedefcounter}{
      name={#1},
      description={p.~\pageref{d:#1}},
      user1={\thedefcounter},
      user2={Definition}
    }
  \fi
}
\newcommand{\eeD}{\end{definition}\glsadd{d\thedefcounter}}
...
\newcommand{\beO}[1][]{
  ...
  \else
    ...
    \newglossaryentry{o\theopcounter}{
      name={#1},
      description={p.~\pageref{op:#1}},
      user1={\theopcounter},
      user2={Open Problem}
    }
  \fi
}
\newcommand{\eeO}{\end{open}\glsadd{o\theopcounter}}

% PRINT COMMAND
\newcommand{\printboth}{
  \printglossary[style=unifiedstyle, title={Definitions and Open Problems}]
}
...
\def\rd{\r_{0}} \def\sd{\s_{0}} \def\dd{D_{0}}\def\vd{v_{0}} \def\zd{z_{0}} \def\xd{\T x_{1}}
\def\ie{i.e. }
...
\def\Rot{R_{ {1}}^{\tilde{2}}} \def\Rto{R_{{2}}^{\tilde{1}}}
...
\newcommand{\GASS}{\text{GASS}}
\newcommand{\IS}{\text{IS}}
\newcommand{\IG}{\text{IG}}
\def\Dt{Descartes type }\def\ME{mathematical epidemiology}
...
\def\endp{endemic fixed point}\def\and{antisymmetric}
...
\def\ye{\bar{\y}}\def\xe{\bar{\x}}\def\we{\bar{\w}}\def\re{\bar{\r}}\def\se{\bar{\s}}
\def\eE{\bar{\e}}\def\con{condition}
...
\def\spfp{strictly positive fixed  point} \def\RUR{rational univariate representation}\def\oth{otherwise}\def\ma{mass action}
\def\how{however}\def\nt{non trivial}\def\fif{forward invariant face}
\def\repF{DFE-reproduction functions}\def\fun{function}\def\num{numerator}
\def\den{denominator}
\def\LVK{Lotka-Volterra-Kolmogorov}\def\mSt{multi-strain model}
\def\mSi{minimal siphon}\def\mtS{multi-strain model}
\def\nota{notation}
